...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template mode="mode-iso19139" match="gn:child" priority="2000">
    ...
    <xsl:param name="labels" select="$iso19139.eamplabels" required="no"/>
    ...
    <xsl:if test="$isEditing and
      ...
      <xsl:variable name="label"
                    select="gn-fn-metadata:getLabel($schema, $name, $iso19139.eamplabels, name(..), '', '')"/>
  ...
  <xsl:template mode="mode-iso19139" priority="200"
    ...
    <xsl:variable name="label" select="gn-fn-metadata:getLabel($schema, name(), $iso19139.eamplabels, name(..), $isoType, $xpath)"/>
  ...
  <xsl:template mode="mode-iso19139" priority="200"
    ...
    <xsl:variable name="labelConfig"
                  select="gn-fn-metadata:getLabel($schema, name(), $iso19139.eamplabels, name(..), $isoType, $xpath)"/>
  ...
  <xsl:template mode="mode-iso19139" match="@uuidref" priority="2000">
    <xsl:call-template name="render-element">
      <xsl:with-param name="label"
                      select="gn-fn-metadata:getLabel($schema, name(..), $iso19139.eamplabels)"/>
  ...
  <xsl:template mode="mode-iso19139" priority="200" match="*[*/@codeList]">
    ...
    <xsl:variable name="labelConfig">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:copy-of select="gn-fn-metadata:getLabel($schema, name(), $iso19139.eamplabels, name(..), '', $xpath)"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    ...
    <xsl:call-template name="render-element">
      ...
      <xsl:with-param name="listOfValues"
                      select="gn-fn-metadata:getCodeListValues($schema, name(*[@codeListValue]), $iso19139.eampcodelists, .)"/>
      <xsl:with-param name="isFirst"
                      select="count(preceding-sibling::*[name() = $elementName]) = 0"/>
    </xsl:call-template>
  ...
  <xsl:template mode="mode-iso19139" match="*[gn:element/gn:text]">
    ...
    <xsl:call-template name="render-element">
      <xsl:with-param name="label"
                      select="gn-fn-metadata:getLabel($schema, name(), $iso19139.eamplabels, name(..), '', $xpath)"/>
      ...
      <xsl:with-param name="editInfo" select="../gn:element"/>
      <xsl:with-param name="listOfValues"
                      select="gn-fn-metadata:getCodeListValues($schema, name(), $iso19139.eampcodelists, .)"/>
      <xsl:with-param name="xpath" select="$xpath"/>
    </xsl:call-template>
  </xsl:template>
